...
def signals_to_bvps_cuda(sig, np_method, params={}):
    """
    Transform an input RGB signal in a BVP signal using a rPPG method (see pyVHR.BVP.methods).
    This method must use np and executes on GPU. You can pass also non-RGB signal but the method used must handle its shape.
    ...
    Args:
        ...
        np_method: a method that comply with the fucntion signature documented in pyVHR.BVP.methods. This method must use np.
    ...
    gpu_sig = np.asarray(sig)
    if len(params) > 0:
        bvps = np_method(gpu_sig, **params)
    else:
        bvps = np_method(gpu_sig)
    r_bvps = np.asnumpy(bvps)
    gpu_sig = None
    bvps = None
    return r_bvps
...
def RGB_sig_to_BVP(windowed_sig, fps, device_type=None, method=None, params={}):
    ...
    Args:
        ...
        method: a method that comply with the fucntion signature documented 
            in pyVHR.BVP.methods. This method must use Numpy if the 'device_type' is 'cpu', Torch if the 'device_type' is 'torch', and np 
