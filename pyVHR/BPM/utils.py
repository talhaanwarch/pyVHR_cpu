...
def Welch_cuda(bvps, fps, minHz=0.65, maxHz=4.0, nfft=2048):
    ...
    Args:
        bvps(flaot32 np.ndarray): BVP signal as float32 Numpy.ndarray with shape [num_estimators, num_frames].
        fps (np.float32): frames per seconds.
        minHz (np.float32): frequency in Hz used to isolate a specific subband [minHz, maxHz] (esclusive).
        maxHz (np.float32): frequency in Hz used to isolate a specific subband [minHz, maxHz] (esclusive).
        nfft (np.int32): number of DFT points, specified as a positive integer.
    Returns:
        Sample frequencies as float32 np.ndarray, and Power spectral density or power spectrum as float32 np.ndarray.
    """
    _, n = bvps.shape
    if n < 256:
        seglength = n
