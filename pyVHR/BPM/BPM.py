...
class BPMcuda:
    ...
    BVP signal must be a float32 np.ndarray with shape [num_estimators, num_frames].
    """
    def __init__(self, data, fps, startTime=0, minHz=0.65, maxHz=4., verb=False):
        """
        Input 'data' is a BVP signal defined as a float32 np.ndarray with shape [num_estimators, num_frames]
        """
    ...
    def BVP_to_BPM(self):
        """
        Return the BPM signal as a float32 np.ndarray with shape [num_estimators, ]. Remember that
        the value returned is on GPU, use np.asnumpy() to transform it to a Numpy ndarray.
