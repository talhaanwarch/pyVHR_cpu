...
def bbox2_GPU(img):
    """
    Args:
        img (np.ndarray): np.ndarray with shape [rows, columns, rgb_channels].
...
class SkinExtractionConvexHull:
    ...
    def extract_skin(self,image, ldmks):
        ...
        if self.device == 'GPU':
            ...
            left_eye_mask = np.asarray(left_eye_mask)
            right_eye_mask = np.asarray(right_eye_mask)
            mounth_mask = np.asarray(mounth_mask)
        ...
        if self.device == 'GPU':
            cropped_skin_im = np.asnumpy(cropped_skin_im)
            skin_image = np.asnumpy(skin_image)
